...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>🔐 BRONX ADMIN PANEL - Custom Key Generator</title>
    <style>
        * {
            margin: 0;
            padding: 0;
        ...
        @keyframes glowPulse {
            0%, 100% { box-shadow: 0 0 30px #ff00ff33, 0 0 50px #00ff4133; }
            50% { box-shadow: 0 0 40px #00ff4133, 0 0 60px #ff00ff33; }
        }
...
<body>
    <div class="container">
        ...
        <div id="loginSection" class="login-container">
            ...
            <div class="input-group">
                ...
                <input type="password" id="password" placeholder="Enter password" id="passwordInput">
            </div>
            <button class="btn btn-primary" style="width: 100%;" onclick="login()">🚀 LOGIN TO PANEL</button>
        ...
        <div id="mainPanel" class="main-panel">
            <div class="header">
                <h1>🔧 BRONX ADMIN PANEL - Custom Key Generator</h1>
            ...
            <div class="section">
                <h2>🔑 CUSTOM KEY GENERATOR <small>Create new API keys with custom limits</small></h2>
                
                <div class="form-grid">
                    <div class="form-group">
                        <label>👤 Owner Name *</label>
                        <input type="text" id="keyOwnerName" placeholder="e.g., John Doe / Company Name">
                    </div>
                    
                    <div class="form-group">
                        <label>📅 Expiry Date</label>
                        <input type="text" id="keyExpiryDate" placeholder="DD-MM-YYYY (leave empty for never)">
                    </div>
                    
                    <div class="form-group">
                        <label>📊 Request Limit</label>
                        <input type="number" id="keyRequestLimit" placeholder="e.g., 100" value="100">
                    </div>
                    
                    <div class="form-group">
                        <label>🏷️ Key Prefix</label>
                        <select id="keyPrefix">
                            <option value="BRONX">BRONX (Standard)</option>
                ...
                <div class="checkbox-group">
                    <input type="checkbox" id="keyUnlimited">
                    <label style="color: #ffff00;">♾️ Unlimited Requests (No limit)</label>
                </div>
                
                <div class="scope-container">
                    <div class="scope-title">🎯 SELECT API SCOPES (Endpoints this key can access)</div>
                    <div class="scope-grid" id="scopeGrid">
                        <!-- Scopes will be populated here -->
                    </div>
                    <div class="checkbox-group" style="margin-top: 10px;">
                        <input type="checkbox" id="selectAllScopes" onchange="toggleAllScopes()">
                        <label style="color: #ff00ff;">⭐ SELECT ALL SCOPES</label>
                    </div>
                </div>
                
                <button class="btn btn-primary" onclick="generateNewKey()" style="width: 100%; padding: 15px;">
            ...
            <!-- Generated Keys List -->
            <div class="section">
                <h2>📋 GENERATED KEYS LIST <small>Manage all custom API keys</small></h2>
                
                <div style="margin-bottom: 20px; display: flex; gap: 10px;">
                    <button class="btn btn-success" onclick="loadKeysList()">🔄 Refresh List</button>
                    <button class="btn btn-warning" onclick="exportKeysData()">📤 Export Keys</button>
                </div>
                
                <div class="key-table-container">
                    <table class="key-table" id="keysTable">
                        <thead>
                            <tr>
                                <th>API Key</th>
                                <th>Owner</th>
            ...
            <div class="section">
                <h2>🔧 CUSTOM API MANAGER <small>10 Slots - Lock/Unlock System</small></h2>
                <p style="color: #ffff00; margin-bottom: 20px; font-size: 14px;">
                    ⚠️ Slots 1-5 are LOCKED (cannot edit/delete). Use slots 6-10 for custom APIs.
                </p>
                
                <div id="customApiList">
                    <!-- Custom APIs will be loaded here -->
                    <p style="color: #888;">Loading custom APIs...</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        let customKeys = {};
        let customAPIs = [
            { id: 1, name: 'Number Info backup ✅', endpoint: 'rajput-api', param: 'num', example: '9876543210', desc: 'india Number Lookup', visible: true, locked: true, realAPI: 'https://rajput-api.vercel.app/search?num={param}' },
            { id: 2, name: 'Vehicle Details Api 🚕', endpoint: 'rc-details', param: 'ca_number', example: 'MH02FZ0555', desc: 'Vehicle RC Details', visible: true, locked: true, realAPI: 'https://bronx-rc-api.vercel.app/?ca_number={param}' },
            { id: 3, name: 'Adhar Detail api', endpoint: 'aadhar-details', param: 'aadhar', example: '393933081942', desc: 'Aadhar Number Lookup', visible: true, locked: true, realAPI: 'https://bronx-adhar-api.vercel.app/aadhar={param}' },
            { id: 4, name: '📧 Email Lookup API', endpoint: 'email-lookup', param: 'mail', example: 'user@gmail.com', desc: 'Email Information', visible: true, locked: true, realAPI: 'https://bronx-mail-api.vercel.app/mail={param}' },
            { id: 5, name: '📲 Telegram Number API', endpoint: 'telegram-num', param: 'id', example: '7530266953', desc: 'Telegram Number Lookup', visible: true, locked: true, realAPI: 'http://45.91.248.51:3000/api/tgnum?id={param}' },
            { id: 6, name: 'Custom API 6', endpoint: '', param: '', example: '', desc: '', visible: false, locked: false, realAPI: '' },
            { id: 7, name: 'Custom API 7', endpoint: '', param: '', example: '', desc: '', visible: false, locked: false, realAPI: '' },
            { id: 8, name: 'Custom API 8', endpoint: '', param: '', example: '', desc: '', visible: false, locked: false, realAPI: '' },
            { id: 9, name: 'Custom API 9', endpoint: '', param: '', example: '', desc: '', visible: false, locked: false, realAPI: '' },
            { id: 10, name: 'Custom API 10', endpoint: '', param: '', example: '', desc: '', visible: false, locked: false, realAPI: '' }
        ];
        
        // ========== INITIALIZATION ==========
        function init() {
            // Load data from localStorage
            const savedKeys = localStorage.getItem('bronx_custom_keys');
            if (savedKeys) {
                customKeys = JSON.parse(savedKeys);
            }
            
            const savedAPIs = localStorage.getItem('bronx_custom_apis');
            if (savedAPIs) {
                customAPIs = JSON.parse(savedAPIs);
            }
            
            // Check if already logged in
            const isLoggedIn = localStorage.getItem('bronx_admin_logged_in');
            if (isLoggedIn === 'true') {
                showMainPanel();
                populateScopeGrid();
                loadKeysList();
                ...
                populateScopeGrid(); // Still populate for login page
            }
        ...
        function populateScopeGrid() {
            ...
            grid.innerHTML = Object.entries(API_ENDPOINTS).map(([key, label]) => `
                <div class="scope-item" onclick="toggleScope('${key}')">
                    <input type="checkbox" id="scope_${key}" value="${key}">
                    <label for="scope_${key}">${label}</label>
                </div>
            `).join('');
        }
        
        function toggleScope(scopeId) {
            const checkbox = document.getElementById('scope_' + scopeId);
            checkbox.checked = !checkbox.checked;
        }
        
        function toggleAllScopes() {
            const selectAll = document.getElementById('selectAllScopes');
            const checkboxes = document.querySelectorAll('#scopeGrid input[type="checkbox"]');
        ...
        function login() {
            ...
            if (username === ADMIN_CREDENTIALS.username && password === ADMIN_CREDENTIALS.password) {
                localStorage.setItem('bronx_admin_logged_in', 'true');
                showMainPanel();
                loadKeysList();
                loadCustomApiList();
                updateStats();
                showToast('✅ Login successful! Welcome BRONX ADMIN!');
            } else {
        ...
        function logout() {
            localStorage.removeItem('bronx_admin_logged_in');
            location.reload();
        }
        ...
        function generateNewKey() {
            const ownerName = document.getElementById('keyOwnerName').value;
            const expiryDate = document.getElementById('keyExpiryDate').value;
            const requestLimit = document.getElementById('keyRequestLimit').value;
            ...
            document.querySelectorAll('#scopeGrid input[type="checkbox"]:checked').forEach(cb => {
                selectedScopes.push(cb.value);
            });
            
            if (selectedScopes.length === 0) {
                showToast('❌ Please select at least one scope!', true);
                return;
            }
            ...
            // Clear form
            document.getElementById('keyOwnerName').value = '';
            document.getElementById('keyExpiryDate').value = '';
            document.getElementById('keyRequestLimit').value = '100';
            document.getElementById('keyUnlimited').checked = false;
            document.querySelectorAll('#scopeGrid input[type="checkbox"]').forEach(cb => cb.checked = false);
            document.getElementById('selectAllScopes').checked = false;
            
            // Show the generated key
            showToast('✅ Key Generated: ' + newKey);
        ...
        function saveKeysToStorage() {
            localStorage.setItem('bronx_custom_keys', JSON.stringify(customKeys));
        }
        
        function saveAPIsToStorage() {
            localStorage.setItem('bronx_custom_apis', JSON.stringify(customAPIs));
        }
        
        // ========== LOAD KEYS LIST ==========
        function loadKeysList() {
            const tbody = document.getElementById('keysTableBody');
            const keys = Object.entries(customKeys);
            
            if (keys.length === 0) {
                tbody.innerHTML = '<tr><td colspan="10" style="text-align: center; padding: 30px;">No custom keys generated yet. Use the generator above to create keys.</td></tr>';
                return;
            }
            
            tbody.innerHTML = keys.map(([key, data]) => {
                const isExpired = data.expiry && new Date(data.expiry.split('-').reverse().join('-')) < new Date();
                ...
                const remaining = data.unlimited ? '∞' : Math.max(0, data.limit - data.used);
                
                return `
                    <tr>
                        <td><code class="key-code">${key}</code></td>
                        <td>${data.name}</td>
                        <td>${data.scopes.includes('*') ? 'ALL' : data.scopes.slice(0, 3).join(', ') + (data.scopes.length > 3 ? '...' : '')}</td>
                        <td>${limitDisplay}</td>
                        <td>${data.used}</td>
                        <td>${remaining}</td>
                        ...
                        <td>
                            <div class="action-buttons">
                                <button class="btn btn-warning btn-small" onclick="copyKey('${key}')">📋 Copy</button>
                                <button class="btn btn-success btn-small" onclick="resetKeyUsage('${key}')">🔄 Reset</button>
                                <button class="btn btn-danger btn-small" onclick="deleteKey('${key}')">🗑️ Delete</button>
                            </div>
                        </td>
                    </tr>
                `;
        ...
        function resetKeyUsage(key) {
            if (customKeys[key]) {
                customKeys[key].used = 0;
                saveKeysToStorage();
                loadKeysList();
                updateStats();
                showToast('✅ Key usage reset successfully!');
            }
        }
        ...
        function updateStats() {
            ...
            keys.forEach(([_, data]) => {
                ...
                if (isExpired) {
                    ...
                    expired++; // Count exhausted as expired for stats
                } else {
                    active++;
                }
            });
        ...
        // ========== CUSTOM API MANAGER ==========
        function loadCustomApiList() {
            const container = document.getElementById('customApiList');
            
            container.innerHTML = `
                <div style="margin-bottom: 15px;">
                    <button class="btn btn-primary" onclick="showApiEditForm()">➕ Add/Edit Custom API</button>
                    <button class="btn btn-success" onclick="saveAllAPIs()">💾 Save All Changes</button>
                </div>
                ${customAPIs.map((api, i) => `
                    <div style="display: flex; align-items: center; gap: 15px; padding: 15px; background: ${api.locked ? '#33000020' : '#1a0033'}; border: 1px solid ${api.locked ? '#ff0000' : '#00ff41'}; border-radius: 10px; margin-bottom: 10px;">
                        <div style="flex: 1;">
                            ...
                            <span style="color: #fff; margin-left: 8px;">${api.name || '(Empty)'}</span>
                            <code style="color: #00ff41; margin-left: 8px;">/${api.endpoint || 'not-set'}</code>
                            <span style="margin-left: 8px; color: ${api.visible ? '#00ff41' : '#ff0000'};">${api.visible ? '👁️ Visible' : '🔒 Hidden'}</span>
                        </div>
                        <div style="display: flex; gap: 8px;">
                            ${!api.locked ? `
                                <button class="btn btn-primary btn-small" onclick="editApiSlot(${i})">✏️ Edit</button>
                                <button class="btn btn-danger btn-small" onclick="clearApiSlot(${i})">🗑️ Clear</button>
                            ` : `
                                <button class="btn btn-small" style="background: #555; cursor: not-allowed;" disabled>🔒 Locked</button>
                            `}
                            <button class="btn btn-warning btn-small" onclick="toggleApiLock(${i})">${api.locked ? '🔓 Unlock' : '🔒 Lock'}</button>
                            <button class="btn btn-success btn-small" onclick="toggleApiVisibility(${i})">${api.visible ? '👁️ Hide' : '👁️ Show'}</button>
                        </div>
                    </div>
                `).join('')}
            `;
        ...
        function toggleApiLock(index) {
            customAPIs[index].locked = !customAPIs[index].locked;
            loadCustomApiList();
        }
        
        function toggleApiVisibility(index) {
            customAPIs[index].visible = !customAPIs[index].visible;
            loadCustomApiList();
        }
        
        function clearApiSlot(index) {
        ...
        function editApiSlot(index) {
            ...
            const newName = prompt('Enter API Name:', api.name);
            if (newName === null) return;
            
            const newEndpoint = prompt('Enter Endpoint (e.g., myapi):', api.endpoint);
            if (newEndpoint === null) return;
            
            const newParam = prompt('Enter Parameter name:', api.param);
            if (newParam === null) return;
            
            const newExample = prompt('Enter Example value:', api.example);
            if (newExample === null) return;
            
            const newDesc = prompt('Enter Description:', api.desc);
            if (newDesc === null) return;
            
            const newRealApi = prompt('Enter Real API URL (use {param}):', api.realAPI);
            if (newRealApi === null) return;
        ...
        function showApiEditForm() {
            const slot = prompt('Enter slot number (1-10):');
            if (!slot) return;
            
            const index = parseInt(slot) - 1;
            if (index < 0 || index >= 10) {
                showToast('❌ Invalid slot number!', true);
                return;
            }
        ...
        function saveAllAPIs() {
            saveAPIsToStorage();
            showToast('✅ All APIs saved to storage!');
        }
        
        // ========== EXPORT KEYS ==========
        function exportKeysData() {
            const data = {
                keys: customKeys,
                apis: customAPIs,
            ...
            a.href = url;
            a.download = `bronx_keys_export_${Date.now()}.json`;
            a.click();
        ...
        // If accessing from URL with ?admin=true, bypass login (for development)

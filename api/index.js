    ...
    <div class="container">
        ...
        <div class="admin-panel">
            ...
            <div style="display: flex; gap: 15px; align-items: center; margin-bottom: 20px;">
                <button onclick="saveCustomAPI()" style="padding: 12px 20px; background: linear-gradient(45deg, #ff00ff, #00ff41); border: none; border-radius: 10px; color: #000; font-weight: bold; cursor: pointer;">💾 Save API</button>
                <button onclick="loadAPIToSlot()" style="padding: 12px 20px; background: linear-gradient(45deg, #ff00ff, #00ff41); border: none; border-radius: 10px; color: #000; font-weight: bold; cursor: pointer;">📂 Load to Form</button>
                <div class="toggle-visibility">
                    <input type="checkbox" id="apiVisibleCheck"> 
                    <label for="apiVisibleCheck">👁️ Visible to Public</label>
                </div>
                <button onclick="toggleAPIVisibility()" style="padding: 12px 20px; background: linear-gradient(45deg, #ff00ff, #00ff41); border: none; border-radius: 10px; color: #000; font-weight: bold; cursor: pointer;">🔄 Toggle Visibility</button>
            </div>
    ...
    <script>
        ...
        async function toggleAPIVisibility() {
            ...
            try {
                ...
                const data = await res.json();
                if (data.success) {
                    showToast(`✅ API ${api.visible ? 'Visible' : 'Hidden'}!`);
                    customAPIs[slot] = api;
                    renderCustomAPIsList();
                }
            } catch (err) {

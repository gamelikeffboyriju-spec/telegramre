// api/index.js - BRONX OSINT v50 - ULTIMATE GOD TIER
...
async function saveToStorage() {
    try {
        ...
        console.log('💾 Saved! Keys:', Object.keys(keyStorage).length, 'Logs:', requestLogs.length);
...
app.get('/test',(req,res)=>{res.json({status:'✅ OK',keys:Object.keys(keyStorage).filter(k=>!keyStorage[k]?.hidden).length,logs:requestLogs.length})});
...
// ========== RENDER FUNCTIONS ==========
function renderLogin(){return `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>BRONX v50 | ADMIN</title>
<link href="https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700;900&family=Rajdhani:wght@500;700&display=swap" rel="stylesheet"><style>
*{margin:0;padding:0;box-sizing:border-box}
body{background:#0a0a0f;display:flex;justify-content:center;align-items:center;min-height:100vh;font-family:'Rajdhani',sans-serif;overflow:hidden}
...
.particle{position:absolute;width:2px;height:2px;background:#fff;border-radius:50%;animation:float 6s infinite;opacity:0}
...
.box::before{content:'BRONX OSINT v50';display:block;text-align:center;font-family:'Orbitron',sans-serif;font-size:11px;letter-spacing:6px;background:linear-gradient(90deg,#8b00ff,#00c8ff,#ff0080);-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-bottom:10px;font-weight:900}
...
.box input:focus{border-color:#8b00ff;box-shadow:0 0 30px rgba(139,0,255,.2),0 0 60px rgba(139,0,255,.05)}
.box input::placeholder{color:#444;letter-spacing:2px}
...
<div class="box"><h2>ADMIN ACCESS</h2><p class="sub">Restricted Area</p>
<input type="text" id="u" placeholder="USERNAME"><input type="password" id="p" placeholder="PASSWORD">
<button class="btn" onclick="login()">Authenticate</button><p class="msg" id="msg"></p><p class="fp">BRONX OSINT v50 · GOD TIER</p></div>
<script>
...
return `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>BRONX v50 | ADMIN PANEL</title>
<link href="https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700;900&family=Rajdhani:wght@400;500;600;700&display=swap" rel="stylesheet"><style>
...
::-webkit-scrollbar{width:3px}::-webkit-scrollbar-track{background:var(--bg)}::-webkit-scrollbar-thumb{background:var(--acc);border-radius:10px}
...
.tb button{background:rgba(139,0,255,.08);color:var(--txt);border:1px solid var(--brd);padding:8px 14px;border-radius:8px;cursor:pointer;font-size:11px;font-family:'Rajdhani',sans-serif;font-weight:600;letter-spacing:1px;transition:.4s}
...
.sc:hover{border-color:var(--acc);box-shadow:0 0 40px rgba(139,0,255,.1)}
.sc .v{font-size:30px;font-weight:900;background:linear-gradient(135deg,var(--acc),var(--acc2));-webkit-background-clip:text;-webkit-text-fill-color:transparent;font-family:'Orbitron',sans-serif}
.sc .l{font-size:9px;color:#555;text-transform:uppercase;letter-spacing:3px;margin-top:4px;font-weight:600}
.tabs{display:flex;gap:6px;margin-bottom:16px;flex-wrap:wrap}
.tab{padding:10px 18px;background:var(--sur);border:1px solid var(--brd);border-radius:10px;color:#666;cursor:pointer;font-size:11px;font-weight:600;letter-spacing:1px;transition:.4s;font-family:'Rajdhani',sans-serif}
.tab:hover{border-color:var(--acc2);color:var(--acc2)}
.tab.on{background:rgba(139,0,255,.1);border-color:var(--acc);color:#fff;box-shadow:0 0 30px rgba(139,0,255,.15)}
.pn{display:none}.pn.on{display:block}
.sn{background:var(--sur);border:1px solid var(--brd);border-radius:16px;padding:20px;margin-bottom:16px}
.sn h3{color:#fff;margin-bottom:14px;font-size:16px;font-weight:700;letter-spacing:2px;font-family:'Orbitron',sans-serif}
.fg{display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:10px}
.fg label{display:block;color:#555;font-size:9px;text-transform:uppercase;letter-spacing:2px;margin-bottom:4px;font-weight:600}
...
.btn2:hover{border-color:var(--acc2);color:var(--acc2)}
...
td{padding:8px 6px;border-bottom:1px solid rgba(255,255,255,.02)}
tr:hover td{background:rgba(139,0,255,.02)}
...
.li span{min-width:70px;font-size:8px}
.li code{color:var(--acc2);font-size:8px}
...
<div class="top"><h1>BRONX_ADMIN_v50</h1><div class="tb"><span style="color:#444;font-size:10px;letter-spacing:1px">[ ${getIndiaDateTime()} ]</span><button onclick="window.open('/')">🏠 HOME</button><button onclick="location.href='/admin'">🚪 LOGOUT</button></div></div>
<div class="ct">
<div class="st">
...
(async function(){const loaded=await loadFromStorage();if(!loaded){initDefaultData();initCustomAPIs()}if(!keyStorage[MASTER_API_KEY])keyStorage[MASTER_API_KEY]=createMasterKey();delete keyStorage['BRONX_ULTRA_MASTER_2026'];scheduleSave();console.log('✅ BRONX OSINT v50 GOD TIER Ready!')})();
